// =====================================
// NEXUS CHAT - AUTH MODULE v3.0 - FIXED
// =====================================
...
function showMainScreen() {
    ...
    if (!chatInitialized) {
        ...
        if (window.initMediaModule) window.initMediaModule();
    }
...
async function sendOTP() {
    const email = emailInput.value.trim();
    if (!email) { 
        showAuthMessage('Enter your email', 'error'); 
        return; 
    }
...
async function verifyOTP() {
    ...
    if (!email || code.length !== 6) {
        showAuthMessage('Enter your email and complete 6-digit OTP', 'error'); 
        return;
    }
...
function connectSocket() {
    ...
    window.socket.on('user-offline', (data) => {
        ...
        if (window.handleUserStatusUpdate) window.handleUserStatusUpdate({ email, online: false, lastSeen });
    });
    ...
    window.socket.on('user-status-update', (data) => {
    ...
    window.socket.on('security-alert', (data) => {
        ...
        if (banner) {
            ...
            banner.innerHTML = `<i class="fas fa-shield-alt"></i> ${data.msg} (${data.ip || 'Unknown'}) <a href="#" style="color:white; text-decoration:underline;">Review</a>`;
...
function logoutUser() {
    ...
    Object.keys(localStorage).forEach(k => { 
        if (k.startsWith('nexus_chat_')) localStorage.removeItem(k); 
    });
    showAuthScreen();
    showToast('Logged out successfully', 'info');
}

// ===== TIME FORMATTING - FIXED (Pakistan Time) =====
function formatTime(timestamp) {
    ...
    if (date.toDateString() === yesterday.toDateString()) {
        return 'Yesterday ' + date.toLocaleTimeString('en-PK', options);
    }
...
function formatLastSeen(timestamp) {
    ...
    return `last seen on ${date.toLocaleDateString('en-PK', { 
        timeZone: 'Asia/Karachi', 
        day: 'numeric', 
        month: 'short' 
    })} at ${timeStr}`;
